...
function normalizeLicensePreset(preset) {
  const raw = String(preset || "mit_arr").toLowerCase();
  if (["mit_arr", "arr_only", "apache_ccby"].includes(raw)) return raw;
  return "mit_arr";
}
...
function implementFeature(input) {
  ...
  const licensePreset = normalizeLicensePreset(input.licensePreset || "mit_arr");
...
app.post("/api/create-project", (req, res) => {
  ...
  const licensePreset = normalizeLicensePreset(body.licensePreset || "mit_arr");
